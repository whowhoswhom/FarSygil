  ...
  return (
    <main className="page-shell fs-view flex flex-col gap-8 pb-5 text-[var(--ink-1)]">
      <PageMasthead
        ...
        title="Health"
        sub="Local Apple Health metrics only. Missing values stay empty until an export writes real rows into SQLite."
        actions={
          <div className="dashboard-shell-card w-full p-4 sm:w-64">
            <p className="fs-eyebrow mb-3">Local source</p>
            <div className="grid grid-cols-2 gap-3">
              <StatusCell
                label="Metric rows"
                value={summary.metricRows.toLocaleString()}
              />
              <StatusCell
                label="Latest metric"
                value={summary.latestMetricDate ?? "--"}
              />
            </div>
          </div>
        }
